    ...
    bindPetTooltips: function() {
		...
		Tooltip.factory('[data-pet-ability]', {
			...
			onShow: function(event) {
				...
				var petInfo = (typeof info != 'undefined') ? "?data=" + encodeURIComponent(info) : "";

				if(typeof abilityData != 'undefined') {
					Tooltip.show(node, '/pet/ability/' + abilityData + '/tooltip' + petInfo, true);
				}
			}
		});
	...
	bindItemTooltips: function() {
		Tooltip.factory('a[href*="/item/"], [data-item]', {
			...
			onShow: function(event) {
				...
				var query = "";
				if(id){
					if(node.search){ query = node.search; }
	...
	bindCharacterTooltips: function() {
		Tooltip.factory('a[href*="/character/"]', {
			...
			onShow: function(event) {
				...
				if (node.href == 'javascript:;' || node.href.indexOf('#') == 0 || node.rel == 'np' || node.href.indexOf('/vault/') != -1)
					return;

				var href = $node.attr('href').replace(Core.baseUrl +'/character/', "").split('/');

				if (location.href.toLowerCase().indexOf('/'+ href[1].toLowerCase() +'/') != -1 && this.rel != 'allow')
					return;

				Tooltip.show(node, '/character/'+ encodeURIComponent(href[0]) +'/'+ href[1] +'/tooltip', true);
			}
		});
	},
    ...
    bindGuildTooltips: function() {
        Tooltip.factory('a[href*="/guild/"]', {
			...
			onShow:	function(event) {
				...
				if (node.href == 'javascript:;' || node.href.indexOf('#') == 0 || node.rel == 'np' )
					return;
				...
				Tooltip.show(node, '/guild/'+ encodeURIComponent(href[0]) +'/'+ href[1] +'/tooltip', true);
			}
        });
    },
		...
		if (map.length > 0) {
			...
			for (var i = 0, len = map.length; i < len; ++i) {
				...
				if (
					...
					($.inArray(Core.region, site.regions) < 0) ||
					!site.urls[type]
				) {
					continue;
				}
